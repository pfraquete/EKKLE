...
import { updateMeetingReport } from '@/actions/meetings'
...
export function EditMeetingForm({ members, meeting, redirectPath = '/membro/minha-celula/reunioes' }: Props) {
    ...
    const [checklist, setChecklist] = useState({
        icebreaker: report?.has_icebreaker ?? false,
        worship: report?.has_worship ?? false,
        word: report?.has_word ?? false,
        prayer: report?.has_prayer ?? false,
        snack: report?.has_snack ?? false
    })

    // Attendance - initialize from existing data
    const presentIds = new Set(
        meeting.attendance
            .filter(a => a.profile_id && a.status === 'PRESENT')
            .map(a => a.profile_id!)
    )
    const [attendance, setAttendance] = useState<Record<string, boolean>>(
        Object.fromEntries(members.map(m => [m.id, presentIds.has(m.id)]))
    )

    // Visitors - initialize from existing data
    const existingVisitors = meeting.attendance
        .filter(a => a.visitor_name)
        .map(a => ({ name: a.visitor_name!, phone: a.visitor_phone || '' }))
    const [visitors, setVisitors] = useState<{ name: string; phone: string }[]>(existingVisitors)
    const [newVisitor, setNewVisitor] = useState({ name: '', phone: '' })

    // Numbers
    const [visitorsCount, setVisitorsCount] = useState(report?.visitors_count ?? existingVisitors.length)
    const [decisionsCount, setDecisionsCount] = useState(report?.decisions_count ?? 0)
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            await updateMeetingReport(payload)
            router.push(`${redirectPath}/${meeting.id}`)
            router.refresh()
        } catch (error) {
            console.error(error)
        ...
        <div className="space-y-6 max-w-lg mx-auto pb-32 bg-zinc-950 min-h-screen p-4 rounded-[2.5rem]">
            ...
            <div className="flex items-center justify-between py-2">
                <div className="flex items-center gap-3">
                    ...
                    <h1 className="text-xl font-black text-white">Editar Reunião</h1>
                </div>
            </div>
                            ...
                            <div key={i} className="flex items-center justify-between p-3 bg-blue-500/10 border border-blue-500/20 rounded-xl">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        {v.phone && <div className="text-xs text-blue-400">{v.phone}</div>}
                                    </div>
                                </div>
