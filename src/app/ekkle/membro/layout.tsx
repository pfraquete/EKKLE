...
export default async function EkkleMembroLayout({
  ...
  if (profile && !isEkkleHubUser(profile)) {
    // User has a real church - they shouldn't be in /ekkle/membro
    redirect('/membro')
  }
    ...
    <div className="min-h-screen flex flex-col bg-background text-foreground selection:bg-primary/20">
      ...
      <header className="border-b border-border/40 bg-background/95 backdrop-blur-xl sticky top-0 z-50">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-3 sm:py-4">
          <div className="flex items-center justify-between gap-3">
            {/* Logo and Ekkle Name */}
            <Link href="/ekkle/membro" className="flex items-center gap-2 sm:gap-4 group min-w-0 flex-1">
              <div className="relative w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl overflow-hidden shadow-lg group-hover:scale-105 transition-all duration-300 border border-primary/20 flex-shrink-0 bg-primary/10 flex items-center justify-center">
                <Image
                  src="/logo.png"
                  alt="Ekkle"
                  width={36}
              ...
              <div className="flex flex-col min-w-0">
                <span className="font-black text-base sm:text-xl tracking-tighter uppercase leading-none mb-0.5 truncate">Ekkle</span>
                <span className="text-[8px] sm:text-[9px] font-black uppercase tracking-[0.2em] sm:tracking-[0.3em] text-primary/80 hidden sm:block">Encontre sua comunidade</span>
              </div>
            </Link>
            ...
            <div className="flex items-center gap-2 sm:gap-4 lg:gap-6 flex-shrink-0">
              <div className="flex items-center gap-2 sm:gap-5 pl-2 sm:pl-6 border-l border-border/50">
                ...
                <form action="/api/auth/signout" method="post">
                  <button
                    type="submit"
                    className="p-2.5 sm:p-3 bg-muted/30 rounded-xl sm:rounded-2xl text-muted-foreground hover:bg-destructive hover:text-destructive-foreground hover:shadow-xl hover:shadow-destructive/20 transition-all duration-300 group"
                    title="Sair"
                  >
      ...
      <div className="flex-1 flex max-w-[1600px] mx-auto w-full">
        ...
        <aside className="w-80 hidden lg:block py-12 px-8">
          <EkkleSidebarNav profile={profile} />
        </aside>
