...
import { getBadgeById, updateBadge, type Badge } from '@/actions/badges'
...
export default function EditarSeloPage() {
  ...
  async function loadBadge() {
    setLoading(true)
    const { data, error } = await getBadgeById(badgeId)
    if (error) {
      toast.error(error)
    ...
    if (data) {
      setFormData({
        ...
        opening_page: data.opening_page,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const { data, error } = await updateBadge(badgeId, form)
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center">
          ...
          <p className="text-gray-text-muted mt-2">Carregando selo...</p>
        </div>
      </div>
    )
  }
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-text-muted mt-1">
            Atualize as informações do selo
          </p>
        </div>
      </div>
